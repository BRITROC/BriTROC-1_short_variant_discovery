...
archival_non_tp53$Consequence = factor(archival_non_tp53$Consequence,
                                       levels=c(
                                         'frameshift_variant',
                                         'inframe_deletion',
                                         'inframe_insertion',
...
archival_non_tp53 = archival_non_tp53 %>% dplyr::arrange(patient_id,Consequence)

archival_non_tp53 = archival_non_tp53 %>% dplyr::filter(`#Uploaded_variation`!='chr17_41231352_T/C')
...
archival_non_tp53 =
  archival_non_tp53 %>% dplyr::select(patient_id,SYMBOL, `#Uploaded_variation`, Existing_variation, Consequence) %>%
  unique()

# remove synonymous and intron variants
archival_non_tp53 = 
  archival_non_tp53 %>% dplyr::filter(!Consequence %in% c('intron_variant','synonymous_variant')) %>%
  unique()

somatic_samples_with_no_mutations = dplyr::anti_join(
  sequenced_samples, archival_non_tp53, by=c('fk_britroc_number'='patient_id')
)
...
somatic_samples_with_mutations = archival_non_tp53 %>% dplyr::select(patient_id,SYMBOL,Consequence)
...
ComplexHeatmap::oncoPrint(
  ...
  alter_fun = alter_fun,
  row_order = c('BRCA1','BRCA2','FANCM','BRIP1','BARD1','RAD51D')
