...
filtered_germline_variants$POS =
        dplyr::if_else(
                nchar(filtered_germline_variants$REF) > 1 | nchar(filtered_germline_variants$ALT) > 1,
                filtered_germline_variants$POS - 1,
                filtered_germline_variants$POS
        )
...
final_germline_variants$POS =
        dplyr::if_else(
                nchar(final_germline_variants$REF) > 1 | nchar(final_germline_variants$ALT) > 1,
                final_germline_variants$POS + 1,
                final_germline_variants$POS
        )

final_germline_variants %>% dplyr::select(`#CHROM`,POS,REF,ALT) %>% unique() %>% print()
