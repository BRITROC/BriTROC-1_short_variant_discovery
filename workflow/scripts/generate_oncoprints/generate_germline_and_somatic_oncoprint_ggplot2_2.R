...
generate_somatic_oncoprint = function(somatic_variants, somatic_oncoprint_output_file, gene_set_analysed) {
	...
	all_genes =  tibble::tibble(names=gene_set_analysed) %>% dplyr::filter(names %in% somatic_variants$gene_symbol_tumour_type)
	...
	genes_with_variants = somatic_variants_tmp %>% dplyr::filter(!is.na(variant_type)) %>%  
		dplyr::filter(!gene_symbol_tumour_type %in% c('RAD51L3-RFFL','SHOE')) %>% dplyr::pull(gene_symbol_tumour_type) %>% unique()
	print(genes_with_variants)
	rm(somatic_variants_tmp)

	# remove redundant genes
	somatic_variants = somatic_variants %>% dplyr::filter(!gene_symbol_tumour_type %in% c('RAD51L3-RFFL'))
	...
	somatic_variants = somatic_variants %>% tibble::add_row(
		patient_id=191,
		variant_type='no mutation',
		gene_symbol_tumour_type='TP53_archival'
	)

	somatic_variants = somatic_variants %>% tibble::add_row(
		patient_id=191,
		variant_type='no mutation',
		gene_symbol_tumour_type='TP53_relapse'
	)

	somatic_variants = somatic_variants %>% tibble::add_row(
		patient_id=191,
		variant_type='no mutation',
		gene_symbol_tumour_type='BARD1_relapse'
	)
	...
	tumour_type_tibble = tibble::tibble(tumour_type=c('diagnosis','relapse'))

	germline_variants = dplyr::inner_join(
			germline_variants,
			tumour_type_tibble,
			by=character()
		)

	germline_variants = germline_variants %>% tidyr::unite(gene_symbol, tumour_type, col='gene_symbol_tumour_type', sep=' ', remove=TRUE)
	...
	gene_mutation_table_tmp = tibble::tibble(
		gene_symbol_tumour_type=somatic_variants$gene_symbol_tumour_type %>% unique()
	)
	...
	no_mutation_table = 
		dplyr::anti_join(
			no_mutation_table,
			somatic_variants,
			by=c('patient_id','gene_symbol_tumour_type')
		)
	...
	somatic_variants$gene_symbol_tumour_type = 
		somatic_variants$gene_symbol_tumour_type %>% stringr::str_replace('_archival',' diagnosis')

	somatic_variants$gene_symbol_tumour_type = 
		somatic_variants$gene_symbol_tumour_type %>% stringr::str_replace('_relapse',' relapse')
	...
	somatic_variants$gene_symbol_tumour_type = somatic_variants$gene_symbol_tumour_type %>% as.factor()

	somatic_variants$variant_type_TP53 = 
		dplyr::if_else(
			somatic_variants$gene_symbol_tumour_type %in% c('TP53 diagnosis','TP53 relapse'),
	...
	somatic_variants$variant_type_BRCA1 = 
		dplyr::if_else(
		somatic_variants$gene_symbol_tumour_type %in% c('BRCA1 diagnosis','BRCA1 relapse'),
		somatic_variants$variant_type %>% as.character(),
		'no mutation'
	)
	...
	somatic_variants$variant_type_BRCA2 = 
		dplyr::if_else(
			somatic_variants$gene_symbol_tumour_type %in% c('BRCA2 diagnosis','BRCA2 relapse'),
			somatic_variants$variant_type %>% as.character(),
			'no mutation'
		)
	...
	gene_symbol_tumour_type_percentages = 
		somatic_variants %>%
		dplyr::filter(variant_type!='no mutation') %>%
		dplyr::group_by(gene_symbol_tumour_type) %>%
		dplyr::select(gene_symbol_tumour_type) %>% 
		table(gene_symbol_tumour_type = .) %>%
		data.frame() %>%
	...
	somatic_variants %>% dplyr::filter(grepl('BRCA1',gene_symbol_tumour_type)) %>% print()
	#quit()
	...
	p1 = ggplot(somatic_variants, aes(x=patient_id, y=as.numeric(gene_symbol_tumour_type))) +
		...
		ylab('') +
                scale_y_continuous(
			breaks = 1:length(somatic_variants$gene_symbol_tumour_type %>% as.factor() %>% levels()),
			labels = 
				somatic_variants$gene_symbol_tumour_type %>% as.factor() %>% levels(),
				sec.axis = sec_axis(
					~.,
					breaks = 1:length(somatic_variants$gene_symbol_tumour_type %>% as.factor %>% levels()),
					labels = gene_symbol_tumour_type_percentages
					)
		)
