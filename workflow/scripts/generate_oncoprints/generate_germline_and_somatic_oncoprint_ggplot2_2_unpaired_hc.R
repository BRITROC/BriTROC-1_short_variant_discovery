...
generate_somatic_oncoprint = function(somatic_variants, somatic_oncoprint_output_file, gene_set_analysed) {
	...
	germline_variants$HGVS_united = dplyr::coalesce(germline_variants$protein_effect, germline_variants$cdna_effect)
	germline_variants = germline_variants %>% tidyr::unite(HGVS_united2, gene_symbol, HGVS_united, sep=':', remove=FALSE)
	germline_variants %>% dplyr::select(gene_symbol,protein_effect,cdna_effect) %>% unique() %>% print()
	readr::write_tsv(germline_variants %>% dplyr::select(HGVS_united2) %>% unique(), 'tmp.txt')
	
	# NB: Additional MTBP filtering is not performed here as that happended previously
	# MTBP job code: AMPLICONSEQ_GERMLINE	
	...
	germline_variants$variant_type %>% as.factor() %>% levels() %>% print()
	...
	p_final = p_final + plot_annotation(
		title = 'BriTROC-1: octopus somatic (bold) + HaplotypeCaller germline (pale) variants',
