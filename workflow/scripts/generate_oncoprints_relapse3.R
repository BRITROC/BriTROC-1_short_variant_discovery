...
relapse_non_tp53 = relapse_non_tp53 %>% 
...
relapse_non_tp53 = relapse_non_tp53 %>% dplyr::select(patient_id,Consequence,SYMBOL)
...
print(relapse_non_tp53 %>% dplyr::select(patient_id, SYMBOL, Consequence), n=Inf)

relapse_variants = rbind(relapse_tp53, relapse_non_tp53, germline_variants)
...
relapse_variants =
  relapse_variants %>% dplyr::select(patient_id,SYMBOL, Consequence) %>%
  unique()
...
pdf('relapse_britroc_oncoprint_germline_and_somatic.pdf')
