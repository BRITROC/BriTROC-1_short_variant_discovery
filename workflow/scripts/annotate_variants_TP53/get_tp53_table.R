...
# read in filtered archival TP53 variants
archival_tp53_variants = readr::read_tsv(snakemake@input[['filtered_archival_TP53_variants']], na='NA')
# read in filtered relapse TP53 variants
relapse_tp53_variants = readr::read_tsv(snakemake@input[['filtered_relapse_TP53_variants']], na='NA')

# bind variants
all_tp53_variants = rbind(archival_tp53_variants,relapse_tp53_variants)

# determine the frequency of each TP53 variant with respect to the patient number
tp53_freq = all_tp53_variants %>% dplyr::group_by(fk_britroc_number,`#Uploaded_variation`) %>% dplyr::summarise(n=dplyr::n()) %>% dplyr::ungroup()

# Add frequency information to the original TP53 variants table 
all_tp53_variants = dplyr::inner_join(all_tp53_variants,tp53_freq, by=c('fk_britroc_number','#Uploaded_variation'))

# arrange the variants by patient number and by frequency and remove the frequency column
...
# introduce changes relating to sample mislabelling
...
# write to unique entries to file
readr::write_tsv(
	all_tp53_variants %>% 
	dplyr::select(fk_britroc_number,sample_id,`#Uploaded_variation`,Consequence,CLIN_SIG,DOMAINS,type,QUAL,mean_MAF,MAF_diff) %>% 
	unique(),
	snakemake@output[[1]]
	)
