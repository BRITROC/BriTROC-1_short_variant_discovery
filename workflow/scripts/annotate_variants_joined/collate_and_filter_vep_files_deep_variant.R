...
annotations = readr::read_tsv(snakemake@input[[1]], comment='##')
...
annotations %>% .$HGVS_united %>% print()

annotations = annotations %>% dplyr::arrange(HGVS_united)

readr::write_tsv(annotations %>% dplyr::select(HGVS_united) %>% unique(), 'results/variant_analysis/unmatched_germline/panel_6_28/deepvariant/all_patients.vep.HGVS.txt')
quit()
