...
somatic_metadata = readr::read_tsv(snakemake@input[['metadata_sheet']]) %>% dplyr::filter(fk_britroc_number==patient_id)
...
print(square_vcf_ft)
print(libraries)
#print(tibble::as_tibble(square_vcf_ft) %>% dplyr::select(INFO), n=Inf, width=Inf)
#print(tibble::as_tibble(square_vcf_ft) %>% dplyr::select(FORMAT), n=Inf, width=Inf)

# different data structures are needed because different variant types have different numbers of sub fields in their FORMAT field
