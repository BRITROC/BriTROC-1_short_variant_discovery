rule filter_octopus_raw_calls:
	...
	output: 'results/variant_analysis/cohort/{patient_id}.{nonoverlapping_id}.filtered.vcf'
...
rule filter_calls2:
	...
	output: 
		tumour_vcf='results/variant_analysis/cohort/{patient_id}.{nonoverlapping_id}.filtered2.vcf',
	threads: 4
...
rule bgzip_vcf:
	...
	output: 
		compressed_vcf=temp('results/variant_analysis/cohort/{patient_id}.{nonoverlapping_id}.filtered2.vcf.gz')
...
rule index_compressed_vcf:
	...
	output: 'results/variant_analysis/cohort/{patient_id}.{nonoverlapping_id}.filtered2.vcf.gz.csi'
	shell: '/home/bioinformatics/software/bcftools/bcftools-1.10.2/bin/bcftools index {input}'

rule concat_vcfs:
	input: 
		compressed_vcfs=lambda wildcards: expand('results/variant_analysis/cohort/{patient_id}.{nonoverlapping_id}.filtered2.vcf.gz', nonoverlapping_id=[1,2,3,4], patient_id=wildcards.patient_id),
		compressed_vcf_indexes=lambda wildcards: expand('results/variant_analysis/cohort/{patient_id}.{nonoverlapping_id}.filtered2.vcf.gz.csi', nonoverlapping_id=[1,2,3,4], patient_id=wildcards.patient_id)
	wildcard_constraints:
		sample='(IM_[0-9]+|JBLAB-[0-9]+)',
		patient_id='[0-9]+'
	output: protected('results/variant_analysis/cohort/{patient_id}.filtered2.vcf')
