...
rule octopus:
	...
	run:
			shell ('../octopus/bin/octopus \
				...
				--forest-model resources/germline.v0.7.2.forest\
				--threads \
				-w temp/ \
...
rule bgzip_vcf:
	input: 'results/tumour_sample_vcfs_octopus/{sample}.{nonoverlapping_id}.filtered2.vcf'
	output: 
		compressed_vcf=temp('results/tumour_sample_vcfs_octopus/{sample}.{nonoverlapping_id}.filtered2.vcf.gz')
	shell: '/home/bioinformatics/software/htslib/htslib-1.6/bin/bgzip < {input} > {output.compressed_vcf}'

rule index_compressed_vcf:
	input: 'results/tumour_sample_vcfs_octopus/{sample}.{nonoverlapping_id}.filtered2.vcf.gz'
	output: 'results/tumour_sample_vcfs_octopus/{sample}.{nonoverlapping_id}.filtered2.vcf.gz.csi'
	shell: '/home/bioinformatics/software/bcftools/bcftools-1.10.2/bin/bcftools index {input}'

rule concat_vcfs:
	input: 
		compressed_vcfs=lambda wildcards: expand('results/tumour_sample_vcfs_octopus/{sample}.{nonoverlapping_id}.filtered2.vcf.gz', nonoverlapping_id=[1,2,3,4], sample=wildcards.sample),
		compressed_vcf_indexes=lambda wildcards: expand('results/tumour_sample_vcfs_octopus/{sample}.{nonoverlapping_id}.filtered2.vcf.gz.csi', nonoverlapping_id=[1,2,3,4], sample=wildcards.sample)
	wildcard_constraints:
		sample='(IM_[0-9]+|JBLAB-[0-9]+)'
	output: 'results/tumour_sample_vcfs_octopus/{sample}.filtered2.vcf'
