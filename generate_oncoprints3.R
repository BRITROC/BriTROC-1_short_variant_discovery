...
archival_non_tp53 = archival_non_tp53 %>% 
...
archival_non_tp53 = archival_non_tp53 %>% dplyr::select(patient_id,Consequence,SYMBOL)
...
archival_variants = rbind(archival_tp53, archival_non_tp53, germline_variants)
...
archival_variants =
  archival_variants %>% dplyr::select(patient_id,SYMBOL,Consequence) %>%
  unique()
...
pdf('archival_britroc_oncoprint_germline_and_somatic.pdf')
